...
function fillCloseModal(){
  ...
  document.getElementById('cv-confirm').addEventListener('click', async ()=>{
    ...
    try{
      ...
      current = null;
      await refreshOpenSales();
      showCloseSaleSummaryPopup(closeResult, closedSale);
    }catch(e){ msg.textContent = e?.message||'Error'; msg.classList.add('error'); }
  });
}

function showCloseSaleSummaryPopup(closeResult, closedSale) {
  const summary = closeResult?.closeSummary || {};
  const invGeneral = Array.isArray(summary.inventoryItems) ? summary.inventoryItems : [];
  const invInvestor = Array.isArray(summary.investorItems) ? summary.investorItems : [];
  ...
  const renderCompactRows = (rows, withInvestor = false) => {
    ...
    return rows.map(r => {
      const sku = escapeHtml(String(r?.sku || 'SIN-SKU'));
      const name = escapeHtml(String(r?.name || 'Sin nombre'));
  ...
  openModal(node);
  node.querySelector('#close-sale-summary-ok')?.addEventListener('click', () => closeModal());
}
