...
function labelFor(id){ if(current && id===current._id){ const p=current?.vehicle?.plate||''; return p?`VENTA · ${p.toUpperCase()}`:id.slice(-6).toUpperCase(); } return id.slice(-6).toUpperCase(); }
function renderTabs(){
  const cont = byId('saleTabs'); if(!cont) return; cont.innerHTML='';
  for(const id of openTabs){
    ...
    tab.querySelector('.close').addEventListener('click', async (e)=>{
      ...
      try{ await API.sales.cancel(id); }catch(e){ alert(e?.message||'No se pudo cancelar'); }
...
async function switchTo(id){ try{ current = await API.sales.get(id); if(!openTabs.includes(id)){ openTabs.push(id); saveTabs(); } renderTabs(); render(); renderWO(); }catch(e){ console.error(e); } }

function renderMini(){ const lp=byId('sv-mini-plate'), ln=byId('sv-mini-name'), lr=byId('sv-mini-phone'); const c=current?.customer||{}, v=current?.vehicle||{}; if(lp) lp.textContent=v.plate||'—'; if(ln) ln.textContent=`Cliente: ${c.name||'—'}`; if(lr) lr.textContent=`Cel: ${c.phone||'—'}`; }

function render(){
  const body = byId('sales-body'), total = byId('sales-total'); if(!body) return; body.innerHTML='';
  (current?.items||[]).forEach(it=>{
    ...
    qty.addEventListener('change', async()=>{ const v=Math.max(1, Number(qty.value||1)||1); current = await API.sales.updateItem(current._id, it._id, { qty:v }); render(); renderWO(); });
    const actions = tr.querySelector('td:last-child');
    const btnEdit = document.createElement('button'); btnEdit.textContent='Editar $'; btnEdit.className='secondary';
    btnEdit.onclick = async()=>{ const v=prompt('Nuevo precio unitario:', String(it.unitPrice||0)); if(v==null) return; current=await API.sales.updateItem(current._id,it._id,{unitPrice:Number(v)||0}); render(); renderWO(); };
...
function renderWO(){ const b=byId('sv-wo-body'); if(!b) return; b.innerHTML=''; for(const it of (current?.items||[])){ const tr=document.createElement('tr'); const td1=document.createElement('td'); const td2=document.createElement('td'); td2.className='t-center'; td1.textContent=it.name||''; td2.textContent=String(it.qty||1); tr.append(td1,td2); b.appendChild(tr); } }

function openModal(node){ const modal=byId('modal'); const slot=byId('modalBody'); const x=byId('modalClose'); if(!modal||!slot||!x) return; slot.replaceChildren(node); modal.classList.remove('hidden'); x.onclick=()=> modal.classList.add('hidden'); }
function closeModal(){ const m=byId('modal'); if(m) m.classList.add('hidden'); }

function parseCode(raw){ if(!raw) return null; let s=String(raw).trim(); try{ if(/^https?:\/\//i.test(s)){ const u=new URL(s); s=u.pathname.split('/').filter(Boolean).pop()||s; } }catch{} const m=s.match(/[a-f0-9]{24}/ig); if(m?.length) return {type:'id',value:m[m.length-1]}; if(/^[A-Z0-9\-_]+$/i.test(s)) return {type:'sku',value:s.toUpperCase()}; return null; }
...
function openQR(){
  ...
  openModal(node);
  const video = node.querySelector('#qr-video'); const sel = node.querySelector('#qr-cam'); const msg = node.querySelector('#qr-msg'); const list = node.querySelector('#qr-history'); const ac = node.querySelector('#qr-autoclose');
  ...
  async function start(){ try{ stop(); const cs={ video: sel.value?{deviceId:{exact:sel.value}}:{facingMode:'environment'}, audio:false }; stream=await navigator.mediaDevices.getUserMedia(cs); video.srcObject=stream; await video.play(); running=true; if(window.BarcodeDetector){ detector=new BarcodeDetector({formats:['qr_code']}); tickNative(); } else { msg.textContent='Tu navegador no soporta API nativa'; } msg.textContent=''; }catch(e){ msg.textContent='No se pudo abrir cámara: '+(e?.message||e?.name||'Desconocido'); } }
  function accept(v){ const t=Date.now(); if(lastCode===v && t-lastTs<1200) return false; lastCode=v; lastTs=t; return true; }
  function onCode(code){ if(!accept(code)) return; const li=document.createElement('li'); li.textContent=code; list.prepend(li); const p=parseCode(code); if(!p){ msg.textContent='Código no reconocido'; return; } (async()=>{ try{ if(p.type==='id'){ current=await API.sales.addItem(current._id,{source:'inventory',refId:p.value,qty:1}); } else { current=await API.sales.addItem(current._id,{source:'inventory',sku:p.value,qty:1}); } render(); renderWO(); if(ac.checked){ stop(); closeModal(); } }catch(e){ msg.textContent=e?.message||'No se pudo agregar'; } })(); }
...
function openAddPicker(){
  ...
  const node=document.createElement('div'); node.className='card'; node.innerHTML=`
    ...
    <div class="row" style="gap:8px;">
      ...
      <button id="go-man" class="secondary">Servicio manual</button>
    </div>`;
  openModal(node);
  ...
  node.querySelector('#go-man').onclick=()=>{ closeModal(); openAddManual(); };
}
...
async function loadQuote(){
  ...
  node.querySelector('#qh-apply').onclick=fetchList; fetchList();
}
...
function openEditCV(){
  ...
  node.querySelector('#sales-save-cv').onclick = async ()=>{
    ...
    try{ current = await API.sales.setCustomerVehicle(current._id, payload); closeModal(); renderMini(); renderTabs(); }catch(e){ alert(e?.message||'No se pudieron guardar los datos'); }
  };
}

let starting=false;
export function initSales(){
  if(window.__SALES_INITED){ return; } window.__SALES_INITED = true;
  const ventas = document.getElementById('tab-ventas'); if(!ventas) return;
  loadTabs(); renderTabs();
  if(openTabs.length){ API.sales.get(openTabs[openTabs.length-1]).then(s=>{ current=s; renderTabs(); render(); renderWO(); }).catch(()=>{}); }
  const btnStart = byId('sales-start'); if(btnStart){ btnStart.onclick = null; }
  btnStart?.addEventListener('click', async (ev)=>{
    if(starting) return; starting=true;
    const btn = ev.currentTarget; if(btn) btn.disabled = true;
    try{ const s = await API.sales.start(); current = s; if(!openTabs.includes(current._id)) openTabs.push(current._id); saveTabs(); renderTabs(); render(); renderWO(); }
  ...
  byId('sales-close')?.addEventListener('click', async ()=>{
    if(!current) return; try{ await API.sales.close(current._id); alert('Venta cerrada'); openTabs = openTabs.filter(x=>x!==current._id); saveTabs(); current=null; renderTabs(); render(); renderWO(); }
    catch(e){ alert(e?.message||'No se pudo cerrar'); }
  });
  byId('sv-print-wo')?.addEventListener('click', async ()=>{ if(!current) return; try{ const fresh = await API.sales.get(current._id); buildWorkOrderPdf(fresh); }catch(e){ alert(e?.message||'No se pudo imprimir OT'); } });
  byId('sales-history')?.addEventListener('click', openSalesHistory);
  byId('sales-print')?.addEventListener('click', async ()=>{ if(!current) return; try{ const fresh = await API.sales.get(current._id); await buildInvoicePdf(fresh); }catch(e){ alert(e?.message||'No se pudo generar la factura'); } });
  byId('sv-loadQuote')?.addEventListener('click', loadQuote);
  byId('sv-edit-cv')?.addEventListener('click', openEditCV);

  API.subscribeSalesEvents((ev)=>{
    const { type, data } = ev || {}; const id = data?.id; if(!id) return;
    if(type==='sale:created'){ if(!openTabs.includes(id)){ openTabs.push(id); saveTabs(); renderTabs(); } }
    else if(type==='sale:updated'){ if(current && current._id===id){ API.sales.get(id).then(s=>{ current=s; render(); renderWO(); }); } }
    else if(type==='sale:closed' || type==='sale:cancelled'){ if(openTabs.includes(id)){ openTabs = openTabs.filter(x=>x!==id); saveTabs(); renderTabs(); if(current && current._id===id){ current=null; render(); renderWO(); } } }
  });
}
...
function openSalesHistory(){
  ...
  openModal(node);
  const from = node.querySelector('#sh-from'); const to = node.querySelector('#sh-to'); const plate= node.querySelector('#sh-plate'); const body=node.querySelector('#sh-body'); const total=node.querySelector('#sh-total');
  async function load(){
    const params = { status:'closed' }; if(from.value) params.from = from.value; if(to.value) params.to = to.value; if(plate.value) params.plate = plate.value.trim();
    const res = await API.sales.list(params); body.innerHTML=''; let acc=0;
    (res?.items||[]).forEach(s => { const tr=document.createElement('tr'); const date=new Date(s.createdAt); const d=date.toLocaleDateString(); tr.innerHTML=`<td>${String(s.number||'').toString().padStart(5,'0')}</td><td>${s?.vehicle?.plate||''}</td><td>${d}</td><td class="t-right">${money(s.total||0)}</td>`; body.appendChild(tr); acc += Number(s.total||0); });
    total.textContent = money(acc);
  }
  node.querySelector('#sh-search').onclick=load; load();
}
