...
/* ============ LIVE HOOK ============ */
window.__salesOnEvent = async (msg)=>{
  try{
    if(!msg || msg.entity!=='sales') return;
    if (typeof current !== 'undefined' && current && current._id){
      const refreshed = await API.sales.get(current._id);
      if (refreshed && refreshed._id === current._id){
        current = refreshed;
        renderSale(); renderMiniCustomer(); renderWorkOrder();
      }
    }
