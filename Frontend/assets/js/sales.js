...
export function initSales(){
  ...
  function renderSaleTabs(){
    ...
    for(const id of openTabs){
      ...
      n.querySelector('.close').onclick = async (e)=>{ e.stopPropagation(); if(!confirm('¿Deseas cancelar la venta?')) return; try{ await API.sales.cancel(id); }catch(err){ alert(err.message||'No se pudo cancelar'); return; } removeOpen(id); if(current && current._id===id){ current=null; renderSale(); renderMiniCustomer(); renderWorkOrder(); } };
      wrap.appendChild(n);
    }
  }
  ...
  $('#sales-add-manual').onclick = async ()=>{
    if(!current) return;
  ...
  $('#sales-print').onclick = async ()=>{ if(!current) return; const doc = await buildWorkOrderPdf(current); doc.save(`venta_${current.number||current._id}.pdf`); };
