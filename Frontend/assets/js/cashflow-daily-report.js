...
function renderAccounts(report) {
  ...
  if (!rows.length) {
    listEl.innerHTML = '<div class="rounded-2xl border border-slate-200 bg-white px-5 py-6 text-sm text-slate-500 shadow-sm">No hay datos disponibles para este cierre.</div>';
    return;
  }

  listEl.innerHTML = rows.map((row) => `
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-lg">
      <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
        <div>
          <h3 class="text-lg font-bold text-slate-900">${escapeHtml(row.name)}</h3>
          <p class="text-xs uppercase tracking-wide text-slate-500">${escapeHtml(row.type || 'Cuenta')}</p>
        </div>
        <div class="text-right">
          <div class="text-xs uppercase tracking-wide text-slate-500">Saldo actual</div>
          <div class="text-2xl font-extrabold text-slate-900">${money(row.currentBalance || 0)}</div>
        </div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-4">
          <div class="text-xs uppercase tracking-wide text-slate-500 font-semibold">Saldo inicial</div>
          <div class="mt-2 text-xl font-bold text-slate-900">${money(row.openingBalance || 0)}</div>
        </div>
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4">
          ...
          <div class="mt-2 text-xl font-bold text-emerald-700">${money(row.income || 0)}</div>
        </div>
        <div class="rounded-xl border border-rose-200 bg-rose-50 px-4 py-4">
          ...
          <div class="mt-2 text-xl font-bold text-rose-700">${money(row.expense || 0)}</div>
        </div>
        <div class="rounded-xl border border-sky-200 bg-sky-50 px-4 py-4">
          ...
          <div class="mt-2 text-xl font-bold text-sky-700">${money(row.currentBalance || 0)}</div>
        </div>
      </div>
    </article>
  `).join('');
