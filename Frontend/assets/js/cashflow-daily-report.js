    ...
    <article class="account-report-page rounded-2xl border border-sky-100 p-6 md:p-8 shadow-2xl overflow-hidden">
      <div class="flex flex-col gap-8 flex-1">
        <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <h3 class="text-5xl md:text-6xl font-extrabold text-slate-900">${escapeHtml(row.name)}</h3>
            <p class="mt-4 text-xl uppercase tracking-wide text-slate-500">${escapeHtml(row.type || 'Cuenta')}</p>
          </div>
          <div class="rounded-2xl border border-sky-100 bg-white/90 px-6 py-6 shadow-sm">
            <div class="text-lg uppercase tracking-wide text-slate-500 font-semibold">Saldo actual</div>
            <div class="mt-3 text-5xl md:text-6xl font-extrabold text-slate-900">${money(row.currentBalance || 0)}</div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-5 flex-1 content-start">
          <div class="account-metric-card rounded-2xl border border-slate-200 bg-white px-6 py-7 shadow-sm flex flex-col justify-center">
            <div class="text-lg uppercase tracking-wide text-slate-500 font-semibold">Saldo inicial</div>
            <div class="mt-4 text-5xl md:text-6xl font-extrabold text-slate-900">${money(row.openingBalance || 0)}</div>
          </div>
          <div class="account-metric-card rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-7 shadow-sm flex flex-col justify-center">
            <div class="text-lg uppercase tracking-wide text-emerald-700 font-semibold">Ingresos</div>
            <div class="mt-4 text-5xl md:text-6xl font-extrabold text-emerald-700">${money(row.income || 0)}</div>
          </div>
          <div class="account-metric-card account-expense-card rounded-2xl border px-6 py-7 shadow-sm flex flex-col justify-center">
            <div class="text-lg uppercase tracking-wide text-rose-700 font-semibold">Salidas (egresos)</div>
            <div class="mt-4 text-5xl md:text-6xl font-extrabold text-rose-700">${money(row.expense || 0)}</div>
          </div>
          <div class="account-metric-card rounded-2xl border border-sky-200 bg-sky-50 px-6 py-7 shadow-sm flex flex-col justify-center">
            <div class="text-lg uppercase tracking-wide text-sky-700 font-semibold">Saldo final</div>
            <div class="mt-4 text-5xl md:text-6xl font-extrabold text-sky-700">${money(row.currentBalance || 0)}</div>
          </div>
        </div>
      </div>
    </article>
