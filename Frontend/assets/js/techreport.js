...
// Observa cambios de pestaña para lazy load (fallback si se recarga directamente en esa pestaña)
if (typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => {
    const tabBtn = document.querySelector('.tabs button[data-tab="reporte-tecnico"]');
    if(tabBtn){
      tabBtn.addEventListener('click', ()=> setTimeout(()=> initTechReport(), 50));
    }
    if(window.location.hash.includes('reporte-tecnico')){
      initTechReport();
    }
  });
}
