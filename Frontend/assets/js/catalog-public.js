...
async function openDetail(id){
  try{
    ...
    const content=`
      ...
      ${it.brand? `<div style='font-size:12px;opacity:.8;'>Marca: ${it.brand}</div>`:''}
      <div style='display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;'>
        ${imgs.map(im=>`<img src='${im.url}' alt='${im.alt||''}' style='width:120px;height:90px;object-fit:cover;border-radius:6px;' loading='lazy' referrerpolicy='no-referrer'/>`).join('')}
      </div>
      <p style='font-size:13px;line-height:1.35;margin-top:10px;'>${(it.description||'').replace(/</g,'&lt;')}</p>
      <div style='margin-top:10px;font-weight:700;'>Precio: ${money(it.price||0)} ${stockBadge? '• '+stockBadge:''}</div>
      <div style='display:flex;gap:8px;margin-top:14px;justify-content:flex-end;'>
        ${contactBtn}
        <button id='d-add' ${it.stock===0? 'disabled':''}>Agregar al carrito</button>
        <button class='secondary' id='d-close'>Cerrar</button>
      </div>`;
    openModal(content);
...
function ensureModal(){ let m=document.getElementById('public-modal'); if(m) return m; m=document.createElement('div'); m.id='public-modal'; m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.55);display:none;align-items:center;justify-content:center;z-index:1000;'; m.innerHTML='<div id="public-modal-box" style="background:var(--card);color:var(--text);padding:18px 20px;border-radius:12px;max-width:600px;width:92vw;max-height:80vh;overflow:auto;position:relative;"></div>'; document.body.appendChild(m); return m; }
function openModal(html){ const m=ensureModal(); const box=m.querySelector('#public-modal-box'); box.innerHTML= html; m.style.display='flex'; }
...
function openCheckoutModal(){ if(state.cart.size===0){ alert('El carrito está vacío'); return; } let total=0; state.cart.forEach(e=>{ total+=(e.item.price||0)*e.qty; }); const itemsHtml= Array.from(state.cart.values()).map(e=>`<div style='display:flex;justify-content:space-between;font-size:12px;'><span>${e.item.name}</span><span>x${e.qty} • ${money(e.item.price||0)}</span></div>`).join(''); let lastDelivery='pickup'; try{ const saved=localStorage.getItem('catalog:deliveryMethod'); if(['pickup','home-bogota','store'].includes(saved)) lastDelivery=saved; }catch{} openModal(`<h3>Checkout</h3><div style='margin:6px 0 10px;padding:10px;background:var(--card-alt,#0f172a);border-radius:8px;'>${itemsHtml||'<em>Sin items</em>'}<div style='margin-top:6px;font-weight:700;'>Total estimado: ${money(total)}</div><div style='font-size:11px;opacity:.7;'>Pago contra entrega.</div></div><form id='checkoutForm' style='display:flex;flex-direction:column;gap:8px;'><label>Identificación* <input id='co-idNumber' required placeholder='CC / NIT' /></label><label>Nombre* <input id='co-name' required /></label><label>Celular* <input id='co-phone' required /></label><label>Correo <input id='co-email' type='email' /></label><label>Dirección <input id='co-address' /></label><label>Método entrega*<select id='co-delivery' required><option value='pickup' ${lastDelivery==='pickup'?'selected':''}>Recolección</option><option value='home-bogota' ${lastDelivery==='home-bogota'?'selected':''}>Envío Bogotá</option><option value='store' ${lastDelivery==='store'?'selected':''}>Retiro taller</option></select></label><label style='display:flex;align-items:center;gap:6px;font-size:12px;'><input type='checkbox' id='co-install' /> Requiere instalación</label><div id='co-delivery-hint' style='font-size:11px; line-height:1.3; background:var(--card-alt); padding:6px 8px; border-radius:6px;'></div><div id='co-lookup-msg' style='font-size:11px;min-height:14px;'></div><div style='display:flex;gap:8px;justify-content:flex-end;margin-top:4px;'><button type='button' class='secondary' id='co-cancel'>Cancelar</button><button type='submit' id='co-submit'>Confirmar pedido</button></div></form>`); bindCheckoutEvents(); }
