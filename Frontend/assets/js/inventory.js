...
if (__ON_INV_PAGE__) {
  ...
  async function openStockInModal(it){
    // Cargar proveedores, inversores y último precio unitario
    let suppliers = [];
    let investors = [];
    let lastUnitPrice = Number(it?.entryPrice);
    try {
      const [loadedSuppliers, loadedInvestors, freshItem] = await Promise.all([
        request('/api/v1/purchases/suppliers'),
        request('/api/v1/purchases/investors'),
        invAPI.getItem(it._id).catch(() => null)
      ]);
      suppliers = loadedSuppliers;
      investors = loadedInvestors;
      const candidate = Number(freshItem?.lastUnitPrice ?? freshItem?.entryPrice ?? lastUnitPrice);
      if (Number.isFinite(candidate) && candidate > 0) lastUnitPrice = candidate;
    } catch (e) {
      console.error('Error cargando proveedores/inversores:', e);
    }
    if (!(Number.isFinite(lastUnitPrice) && lastUnitPrice > 0)) lastUnitPrice = null;
    const lastPriceHint = lastUnitPrice != null
      ? `Último precio: $${fmtMoney(lastUnitPrice)} (guía, puedes cambiarlo)`
      : 'Precio de compra por cada unidad';
    const lastPriceValue = lastUnitPrice != null ? String(lastUnitPrice) : '';
      ...
      <div class="p-6">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input id="stk-purchase-price" type="number" min="0" step="0.01" value="${lastPriceValue}" placeholder="${lastPriceValue || '0.00'}" class="w-full px-4 py-2 rounded-lg bg-slate-700/50 border border-slate-600/50 text-white theme-light:bg-white theme-light:text-slate-900 theme-light:border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
            <p class="text-xs text-slate-400 theme-light:text-slate-500 mt-1">${lastPriceHint}</p>
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-300 theme-light:text-slate-700 mb-2">
