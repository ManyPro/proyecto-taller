...
async function preview(){
  try {
    ...
    const baseSalaryStr = (el('pl-base')?.value || '').trim();
    ...
    if (!baseSalaryStr) {
      alert('⚠️ Ingresa el sueldo base');
      el('pl-base')?.focus();
      return;
    }
    ...
    if (isNaN(baseSalary) || baseSalary < 0) {
      alert('⚠️ El sueldo base debe ser un número positivo');
      el('pl-base')?.focus();
      return;
    }
        ...
        <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:16px;">
          <div style="margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid var(--border);">
            ...
            <div class="row" style="gap:16px;flex-wrap:wrap;">
              ...
              <div style="font-size:13px;color:var(--muted);">
                <strong>Sueldo base:</strong> ${formatMoney(baseSalary)}
              </div>
            </div>
          </div>
...
async function approve(){
  try {
    ...
    if (!confirm(`¿Aprobar la liquidación de ${technicianName}?\n\nUna vez aprobada, podrás generar el comprobante de pago.`)) {
      return;
    }
    ...
    try {
      ...
      el('pl-result').innerHTML = `
        <div style="padding:16px;background:rgba(16,185,129,0.1);border:1px solid #10b981;border-radius:8px;margin-bottom:16px;">
          ...
          <div style="font-size:13px;color:var(--text);margin-bottom:4px;">
            <strong>Neto a pagar:</strong> ${formatMoney(r.netTotal)}
          </div>
