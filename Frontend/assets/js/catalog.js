...
function renderItems(){
  ...
  if(!grid||!pag) return;
  grid.innerHTML='';
  if(!state.items.length){ grid.innerHTML = `<div class='empty'>No hay items publicados que coincidan.</div>`; pag.innerHTML=''; return; }
  state.items.forEach(it=> grid.appendChild(buildItemCard(it)) );
  renderPagination();
}
...
function updateCartUI(){
  ...
  state.cart.forEach(({ item, qty }) => {
    ...
    row.querySelector('input').onchange = (e) => { const v=parseInt(e.target.value||'1',10); if(v<1) e.target.value='1'; state.cart.get(item.id).qty = Math.max(1,v); updateCartUI(); };
    row.querySelector('[data-rm]').onclick = () => { state.cart.delete(item.id); updateCartUI(); };
...
function bindCheckoutEvents(){
  ...
  async function lookupProfile(force=false){
    ...
    setTimeout(()=>{ lookupMsg.style.opacity='.6'; },2500);
  }
